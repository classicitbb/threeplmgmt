    ...
    <div className="flex min-h-screen bg-gradient-to-br from-background via-background to-muted/30">
      ...
      <div className="flex flex-1 items-center justify-center p-6">
        <div className="w-full max-w-sm space-y-6">
          ...
          {mode === "login" ? (
            <Form {...loginForm}>
              <form className="space-y-4" onSubmit={loginForm.handleSubmit((v) => loginMutation.mutate(v))}>
                <FormField
                  control={loginForm.control}
                  name="email"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Email or user code</FormLabel>
                      <FormControl>
                        <Input {...field} autoComplete="username" placeholder="user@example.com or ADMIN01" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
