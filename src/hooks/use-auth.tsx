/**
 * @file use-auth.tsx — Authentication context: Supabase Auth + demo fallback + RBAC
 *
 * PURPOSE
 * -------
 * Provides AuthContext to the entire app via AuthProvider.
 * Manages Supabase session, profile (from `profiles` table), and role codes (from `user_roles`).
 *
 * SIGN-IN METHODS
 * ---------------
 * Users sign in with one of three identifiers + password:
 *   1. Email         (e.g. admin@warehousewizard.local)
 *   2. User code     (e.g. ADMIN01) — resolved to email via resolve_login_code() RPC
 *   3. Badge code    (e.g. BADGE-ADMIN01) — same RPC handles both formats
 * Identifier type is inferred by contains-"@" and BADGE- prefix checks in LoginPage (App.tsx).
 *
 * DEMO MODE
 * ---------
 * Enabled when: DEV build, VITE_ENABLE_DEMO=true, or hostname matches *.lovable.app
 * (except the production host threeplmgmt.lovable.app, which is explicitly excluded).
 *
 * Demo credentials: any demo email + password "Warehouse123!"
 * Real Supabase Auth is always attempted first. Demo fallback only activates when:
 *   - Supabase returns a schema-query error (local dev without migrations), OR
 *   - network fetch fails (offline/unreachable instance)
 *
 * Demo sessions store the email in localStorage ("warehouse-wizard-demo-session").
 * access_token is a hardcoded "preview-demo-token" — auth.uid() returns NULL in DB
 * functions, so RLS policies and audit events will not resolve a real user.
 *
 * Demo users (all use password Warehouse123!):
 *   admin@warehousewizard.local      ADMIN01 / BADGE-ADMIN01   → all roles
 *   manager@warehousewizard.local    MGR01   / BADGE-MGR01     → manager + clerk + operator
 *   clerk@warehousewizard.local      CLK01   / BADGE-CLK01     → inventory_clerk
 *   operator@warehousewizard.local   OPR01   / BADGE-OPR01     → warehouse_operator
 *   driver@warehousewizard.local     DRV01   / BADGE-DRV01     → dispatch_driver
 *   supervisor@warehousewizard.local SUP01   / BADGE-SUP01     → manager + operator
 *
 * USER APPROVAL
 * -------------
 * New sign-ups require admin approval (profiles.approved = true) before login.
 * RequireAuth in App.tsx renders a "Pending Approval" screen for unapproved users.
 *
 * PROFILE BUNDLE
 * --------------
 * fetchProfileBundle() loads profiles + user_roles in one parallel call.
 * Roles are deduplicated and typed as RoleCode[].
 * hasRole(allowed) returns true if the user holds ANY of the provided codes (OR logic).
 *
 * ROLES
 * -----
 * admin | warehouse_manager | inventory_clerk | warehouse_operator | dispatch_driver
 * Navigation and route guards use hasRole() from this context.
 * Feature flags (module visibility) are managed separately in use-feature-flags.ts.
 */
