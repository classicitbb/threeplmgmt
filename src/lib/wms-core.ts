/**
 * @file wms-core.ts — Central business-logic layer for Warehouse Wizard WMS
 *
 * PURPOSE
 * -------
 * All warehouse operations live here: receiving, putaway, picking, transfers, cycle counts,
 * status changes, location moves, draft receipts, system logs, and client variables.
 * The UI layer (wms-ui.tsx + App.tsx) calls these functions exclusively — no direct Supabase
 * calls exist in UI files.
 *
 * DATABASE ACCESS PATTERN
 * -----------------------
 * `const db = supabase.from.bind(supabase) as (table: string) => any`
 * db() is used for WMS tables not yet in the generated types (supabase/types.ts).
 * Once all enterprise migrations are stable, migrate callers to typed supabase.from<T>().
 * The typed tables (profiles, user_roles, roles) still use direct supabase.from() calls.
 *
 * PALLET LIFECYCLE (inventory_balances.status + pallets.status)
 * -------------------------------------------------------------
 *   receiving  → putaway confirmed → available (is_stored=true, location set)
 *   available  → pick allocated (available_quantity decremented)
 *   available  → picked (qty exhausted after confirmPickTask)
 *   available  → hold | quarantine | damaged | missing (changePalletStatus)
 *   available  → in_transit (dispatchTransfer — location cleared)
 *   in_transit → receiving (receiveTransfer — new putaway_task created)
 *   any status → status_change record in stock_adjustments
 *
 * ARCHIVE PATTERN
 * ---------------
 * Resources use one of two soft-delete strategies:
 *   archiveField: "active"     → active=false hides row (clients, products, warehouses, profiles)
 *   archiveField: "is_hidden"  → is_hidden=true + hidden_at/hidden_reason (zones, locations, packaging)
 * applyArchiveFilter() applies the correct filter; pass includeHidden=true for admin views.
 *
 * LOCATION CODE CONVENTION
 * ------------------------
 * Pattern: {WAREHOUSE}-{ZONE}-{AISLE}-{BAY}-L{LEVEL}  e.g. MAIN-AMB-A-01-L02
 * Generated by the Location Wizard in wms-ui.tsx; stored in locations.code.
 * QR codes replace CODE128 barcodes for codes longer than 20 characters.
 *
 * DRAFT RECEIPTS
 * --------------
 * Drafts store form state as JSON in receipts.notes with _draft: true.
 * Returned-pallet drafts also set _returned: true + returned_pallet_id.
 * completeReceiptFromDraft() routes to completeReturnedPalletDraft() or a fresh
 * createReceiptFlow() based on the _returned flag.
 *
 * AUDIT TRAIL
 * -----------
 * Every mutating operation calls log_audit_event RPC. Failures are console-logged
 * but do NOT block the primary operation (fire-and-forget pattern).
 * Key event types: receipt, putaway, pick, transfer_driver_signoff, transfer_cancelled,
 * status_change, move_task_completed, putaway_reverted_to_draft, count_line_exception.
 *
 * DIRECTED PUTAWAY
 * ----------------
 * directed_putaway_candidates RPC returns ranked location suggestions.
 * The top suggestion is stored on putaway_task; the operator can override it.
 * validatePutawayAssignment() enforces rules (active status, temperature match, capacity,
 * mixed-SKU policy, height limit). Rule failures throw "RULE_VIOLATION: {reason}" —
 * the UI intercepts this prefix to show an override-confirmation dialog.
 *
 * RESOURCE DEFINITIONS
 * --------------------
 * RESOURCE_DEFINITIONS drives the generic ResourcePage CRUD component in wms-ui.tsx.
 * Each entry declares: table, fields (with types/options), roles, archiveField,
 * importable/exportable flags, and helpId for the help sidebar.
 *
 * NAVIGATION
 * ----------
 * The NAVIGATION array is the single source of truth for sidebar links and role-gating.
 * Each entry maps label → route → allowed roles → optional moduleKey (feature flag key).
 * AppShell in wms-ui.tsx filters this array by hasRole() and isEnabled(moduleKey).
 *
 * TASK NUMBER PREFIXES
 * --------------------
 * RCT=receipt  PLT=pallet  PTA=putaway  PKL=pick-list  PKT=pick-task
 * TRF=transfer  MOV=move  CNT=cycle-count  ADJ=adjustment  STS=status-change
 * All generated by buildPalletCode(prefix) = `${prefix}-${Date.now().slice(-8)}`
 */
