...
begin
  ...
  if exists(select 1 from public.warehouses limit 1) then
    raise exception 'Existing warehouse structure found. Run Reset All before using the setup wizard.';
  end if;
  ...
  if coalesce(seed_mode, 'starter_ops') = 'starter_ops' then
